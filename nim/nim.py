...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        if not actions:
            return 0
        # There are available actions, choose and return the one with highest q-value.
        max_val = self.get_q_value(state, actions[0])
    ...
    def choose_action(self, state, epsilon=True):
        ...
        max_val = self.best_future_reward(state)
        for action in actions:
            if self.get_q_value(state, action) == max_val:
                return action
